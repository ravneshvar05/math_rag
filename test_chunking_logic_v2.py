...
def test_smart_chunking():
    ...
    # Include a figure reference in Para 2
    para1 = "This is paragraph 1. " * 20
    para2 = "This is paragraph 2 with reference to Fig 1. " * 15
    ...
    # Mock images
    # Image ID must match the reference "Fig 1" -> "fig_1"
    images = [{'image_id': 'fig_1', 'image_path': 'path/to/img', 'page_number': 1, 'description': 'Test Fig'}]
    ...
    for i, chunk in enumerate(chunks):
        ...
        print(f"  Chunk {i+1} Text Preview: {chunk.text_content[:30]}...")
        ...
        # Check image linking logic
        if "Fig 1" in chunk.text_content:
            if not chunk.images:
                failures.append(f"FAIL: Chunk {i+1} has Ref but missing Image.")
            else:
                print(f"    PASS: Chunk {i+1} correctly linked image.")
        else:
            if chunk.images:
                failures.append(f"FAIL: Chunk {i+1} has NO Ref but incorrectly linked Image.")
            else:
                print(f"    PASS: Chunk {i+1} correctly excluded unrelated image.")
